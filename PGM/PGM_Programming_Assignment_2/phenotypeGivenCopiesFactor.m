...
for i=1:numAlleles
  for j=1:numAlleles
    genotypeId = allelesToGenotypes(i, j);

    idx1 = 1 + 2*(i-1) + (2*numAlleles)*(j-1);
    idx2 = 2 + 2*(i-1) + (2*numAlleles)*(j-1);
    phenotypeFactor.val(idx1) = alphaList(genotypeId);
    phenotypeFactor.val(idx2) = 1 - alphaList(genotypeId);
  end
end
