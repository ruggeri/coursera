...
for valIdxA=1:length(A.val)
  assignA = IndexToAssignment(valIdxA, A.card);
  assignB = assignA(mapB);
  valIdxB = AssignmentToIndex(assignB, B.card);
  B.val(valIdxB) = max(B.val(valIdxB), A.val(valIdxA));
end
